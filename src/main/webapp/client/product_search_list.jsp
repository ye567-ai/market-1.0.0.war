				...
				<table cellspacing="0" class="listcontent">
					<tr>
						<td>
							<h1>商品目录</h1>
							<hr />
							<h1>${bean.searchfield}</h1>&nbsp;&nbsp;&nbsp;&nbsp;共${bean.totalCount}种商品
							<hr />
							<div style="margin-top:20px; margin-bottom:5px">
								<img src="${pageContext.request.contextPath }/client/images/productlist.gif" width="100%" height="34" alt=""/>
							</div>
								...
								<ul>
									<c:if test="${bean.currentPage!=1}">
										<li class="disablepage_p">
											<a class="disablepage_a" href="${pageContext.request.contextPath}/MenuSearchServlet?currentPage=${bean.currentPage-1}&searchfield=${bean.searchfield}"></a>
										</li>
									</c:if>
									<c:if test="${bean.currentPage==1}">
									...
									<c:forEach begin="1" end="${bean.totalPage}" var="pageNum">
										...
										<c:if test="${pageNum!=bean.currentPage}">
											<li><a href="${pageContext.request.contextPath}/MenuSearchServlet?currentPage=${pageNum}&searchfield=${bean.searchfield}">${pageNum}</a>
											</li>
										</c:if>
									</c:forEach>
